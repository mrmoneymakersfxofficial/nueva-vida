...
export default function HomePageClient({ heroSlide, stats: sanityStats, siteSettings, services: sanityServices, testimonials: sanityTestimonials, mentorships: sanityMentorships, facilities: sanityFacilities }: HomePageClientProps) {
  ...
  const heroTitle = heroSlide?.title || 'Cuidamos tu Nueva Vida'
  const heroSubtitle = heroSlide?.subtitle ? plainText(heroSlide.subtitle) : 'Atención médica especializada con tecnología de última generación. Tu salud y bienestar están en las mejores manos.'
  const heroCtaLabel = heroSlide?.ctaLabel || 'Agendar Cita'
  const heroCtaLink = heroSlide?.ctaLink || '/reservas#hero'

  const displayStats = (sanityStats && sanityStats.length > 0)
    ? sanityStats.map(s => ({
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="space-y-4">
                ...
                <h1 {...ve(heroSlide?._id || 'heroSlide-fallback', 'heroSlide', 'title')} className="text-4xl sm:text-5xl lg:text-7xl font-bold text-white leading-tight">
                  {heroTitle}
                </h1>
                <p {...ve(heroSlide?._id || 'heroSlide-fallback', 'heroSlide', 'subtitle')} className="text-lg sm:text-xl text-white/80 max-w-lg leading-relaxed">
                  {heroSubtitle}
                </p>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }}
                ...
                {displayStats.map((stat) => (
                  <div key={stat._id || stat.label} {...ve(stat._id, 'stat', 'value')} className="text-center">
                    <div className="text-2xl sm:text-3xl font-bold text-cyan-light">{stat.value}</div>
                    <div {...ve(stat._id, 'stat', 'label')} className="text-white/50 text-xs sm:text-sm mt-1">{stat.label}</div>
                  </div>
                ))}
              </motion.div>
            </div>
