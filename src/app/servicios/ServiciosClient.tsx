...
/* ═══════════════════════════════════════════════════════════════════
   ICON MAP — Maps icon names from Sanity to Lucide components
   ═══════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════
   HARDCODED FALLBACK DATA — 4 Categorías de Servicios Especializados
   ═══════════════════════════════════════════════════════════════════ */
...
function SubNavbar({ categories }: { categories: DisplayCategory[] }) {
    ...
    <ScrollReveal>
      ...
      >
        <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h3 className="text-2xl sm:text-3xl font-bold text-marine mt-3 leading-tight" {...(isSanity ? ve(category._id!, 'serviceCategory', 'fullTitle') : {})}>
            {category.title}
          </h3>
          <p className="text-marine/60 text-base sm:text-lg mt-3 leading-relaxed max-w-3xl" {...(isSanity ? ve(category._id!, 'serviceCategory', 'lead') : {})}>
            {category.lead}
          </p>
            ...
            <div className={`lg:col-span-3 ${!isEven ? 'lg:order-2' : ''}`}>
              <Accordion type="single" collapsible className="space-y-3">
                {category.services.map((service, i) => {
                  const sanityService = category._sanityServices?.[i]
                  return (
                    <AccordionItem
                      key={i}
                    ...
                    >
                      <AccordionTrigger className="text-marine font-semibold text-left hover:no-underline py-4 text-sm sm:text-base leading-snug [&>svg]:text-cyan [&>svg]:shrink-0 gap-3">
                        <span className="flex items-start gap-3">
                          ...
                          <span {...(sanityService ? ve(sanityService._id, 'service', 'title') : {})}>{service.name}</span>
                        </span>
                      </AccordionTrigger>
                      <AccordionContent className="text-marine/60 text-sm sm:text-base leading-relaxed pb-4 pl-8 sm:pl-[52px]">
                        <span {...(sanityService ? ve(sanityService._id, 'service', 'shortDescription') : {})}>
                          {service.description}
                        </span>
...
export default function ServiciosClient({ categories: sanityCategories, allServices }: ServiciosClientProps) {
  // Map Sanity data to the format the existing UI expects
  // Fall back to hardcoded data if no Sanity data
  const displayCategories: DisplayCategory[] = (sanityCategories && sanityCategories.length > 0)
    ? sanityCategories.map((cat: any) => {
        const catServices = allServices?.filter((s: any) => s.category?._id === cat._id) || []
        return {
          id: cat.slug?.current || cat._id,
          label: cat.name || '',
          shortLabel: cat.shortLabel || cat.name || '',
          icon: iconMap[cat.icon] || Stethoscope,
          color: cat.color || '#00B0F0',
          title: cat.fullTitle || cat.name || '',
          lead: cat.lead || '',
          images: [
            cat.imagePath ? { src: cat.imagePath, alt: cat.fullTitle || cat.name || '' } : null,
            ...(cat.galleryPaths || []).map((p: string) => ({ src: p, alt: cat.name || '' })),
          ].filter(Boolean) as { src: string; alt: string }[],
          services: catServices.map((s: any) => ({
            name: s.title || '',
            description: s.shortDescription || '',
            ctaLabel: 'Agendar esta Consulta',
          })),
          _id: cat._id,
          _sanityServices: catServices,
        }
      })
    : fallbackCategories
  ...
  return (
    <>
      ...
      <section id="servicios-especializados" className="bg-gradient-to-b from-[#FAFCFF] to-white">
        <SubNavbar categories={displayCategories} />
