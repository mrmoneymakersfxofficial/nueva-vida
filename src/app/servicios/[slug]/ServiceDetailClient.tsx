...
export default function ServiceDetailClient({ service: sanityService, slug }: ServiceDetailClientProps) {
  // Determine whether to use Sanity data or fallback
  const isSanity = !!sanityService
  const fallback = servicesData[slug]

  // Resolve display data
  const displayTitle = sanityService?.title || fallback?.title || ''
  const displaySubtitle = sanityService?.subtitle || fallback?.subtitle || ''
  const displayDescription = sanityService?.shortDescription || fallback?.description || ''
  const displayFullDescription = sanityService?.description
    ? plainText(sanityService.description)
    : (fallback?.fullDescription || '')
  const displayImage = sanityService?.imagePath || sanityService?.coverImage?.asset
    ? (sanityService.imagePath || `/api/image?url=${encodeURIComponent(sanityService.coverImage?.asset?._ref || '')}`)
    : (fallback?.image || '/ultrasound-service.jpg')
  const displayPrice = sanityService?.price || fallback?.price || ''
  const displayDuration = sanityService?.duration || fallback?.duration || ''

  // Features: from Sanity subservices or fallback
  const displayFeatures: string[] = sanityService?.subservices?.length > 0
    ? sanityService.subservices.map((s: any) => s.title || '')
    : (fallback?.features || [])

  // Preparation
  const displayPreparation: string[] = sanityService?.preparation?.length > 0
    ? sanityService.preparation
    : (fallback?.preparation || [])

  // FAQs
  const displayFaqs = sanityService?.faqs?.length > 0
    ? sanityService.faqs
    : (fallback?.faqs || [])

  // Not found case
  if (!sanityService && !fallback) {
    return (
  ...
  const serviceId = sanityService?._id
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-4">
              <Badge className="bg-cyan/20 text-cyan-light border-0">{displaySubtitle}</Badge>
            </div>
            <h1 className="text-4xl sm:text-5xl font-bold text-white" {...(isSanity && serviceId ? ve(serviceId, 'service', 'title') : {})}>
              {displayTitle}
            </h1>
            <p className="text-white/80 mt-4 text-lg max-w-2xl" {...(isSanity && serviceId ? ve(serviceId, 'service', 'subtitle') : {})}>
              {displayDescription}
            </p>
          </motion.div>
        </div>
            ...
            <div className="lg:col-span-2 space-y-12">
              <ScrollReveal>
                <div className="relative rounded-2xl overflow-hidden shadow-xl">
                  <Image src={displayImage} alt={displayTitle} width={800} height={450} className="object-cover w-full" />
              ...
              <ScrollReveal delay={0.1}>
                <div className="space-y-4">
                  ...
                  <p className="text-marine/70 leading-relaxed text-lg" {...(isSanity && serviceId ? ve(serviceId, 'service', 'description') : {})}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {displayFeatures.map((feat, idx) => {
                      const subservice = sanityService?.subservices?.[idx]
                      return (
                        <div key={idx} className="flex items-center gap-3 p-4 rounded-xl bg-muted/50" {...(isSanity && subservice ? ve(subservice._id || serviceId, 'subservice', 'title') : {})}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    {displayFaqs.map((faq: any, idx: number) => (
                      <Card key={faq.q || idx} className="border-0 shadow-md bg-muted/30">
                        <CardContent className="p-6 space-y-2">
                          <h3 className="font-semibold text-marine" {...(isSanity && serviceId ? ve(serviceId, 'service', `faqs[${idx}].q`) : {})}>
                            {faq.q}
                          </h3>
                          <p className="text-marine/60 text-sm leading-relaxed" {...(isSanity && serviceId ? ve(serviceId, 'service', `faqs[${idx}].a`) : {})}>
                            {faq.a}
                          </p>
                        </CardContent>
                      </Card>
                ...
                <Card className="border-0 shadow-xl bg-gradient-to-b from-cyan to-royal text-white sticky top-28">
                  <CardContent className="p-8 space-y-6">
                    <div>
                      ...
                      <p className="text-3xl font-bold" {...(isSanity && serviceId ? ve(serviceId, 'service', 'price') : {})}>
                        {displayPrice}
                      </p>
                    </div>
                    <Separator className="bg-white/20" />
                    <div className="flex items-center gap-3 text-sm" {...(isSanity && serviceId ? ve(serviceId, 'service', 'duration') : {})}>
                    ...
                    <MagneticButton href="https://wa.me/51983554248" strength={0.15}>
                      <Button className="w-full bg-[#25D366] hover:bg-[#1DA851] text-white font-semibold rounded-xl py-3 h-auto shadow-lg shadow-[#25D366]/30 transition-all duration-300 hover:shadow-xl hover:shadow-[#25D366]/40">
                ...
                <Card className="border-0 shadow-lg">
                  <CardContent className="p-6 space-y-4">
                    ...
                    <ul className="space-y-3">
                      {displayPreparation.map((prep: string, idx: number) => (
                        <li key={idx} className="flex items-start gap-2 text-sm text-marine/70" {...(isSanity && serviceId ? ve(serviceId, 'service', `preparation[${idx}]`) : {})}>
