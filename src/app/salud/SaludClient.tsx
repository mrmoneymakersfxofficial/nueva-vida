...
// ── Icon mapping for Sanity icon field ──
...
export default function SaludClient({ articles, settings }: SaludClientProps) {
  // Determine if we have CMS data
  const hasCmsArticles = articles && articles.length > 0
  const hasCmsFaqs = settings?.healthFaqs && settings.healthFaqs.length > 0

  // Resolve display articles: map Sanity articles or use hardcoded fallback
  const displayArticles = hasCmsArticles
    ? articles.map((a: any) => ({
        slug: a.slug?.current || a._id,
        icon: (a.icon && iconMap[a.icon]) || Heart,
        title: a.title || '',
        description: a.description || '',
        color: a.color || 'from-cyan-500/10 to-sky-500/10',
        _id: a._id,
      }))
    : fallbackArticles

  // Resolve display FAQs: use Sanity FAQs or hardcoded fallback
  const displayFaqs = hasCmsFaqs ? settings.healthFaqs : fallbackFaqs

  // Resolve hero text
  const heroBadge = settings?.healthPageTitle || 'Portal de Salud'
  const heroTitle = settings?.healthPageTitle || 'Información para tu'
  const heroTitleHighlight = settings?.healthPageDescription || 'bienestar'
  const heroDescription = settings?.healthPageDescription || 'Artículos informativos sobre salud femenina y respuestas a las preguntas más frecuentes para que tomes mejores decisiones sobre tu salud.'

  // Resolve FAQ section headings
  const faqTitle = settings?.healthFaqTitle || 'Preguntas Frecuentes'
  const faqDescription = settings?.healthFaqDescription || 'Encuentra respuestas a las dudas más comunes sobre salud ginecológica.'
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <Badge className="bg-cyan/20 text-cyan-light border-0 mb-4 px-4 py-1"
              {...(settings?._id ? ve(settings._id, 'siteSettings', 'healthPageTitle') : {})}
            >
              {heroBadge}
            </Badge>
            ...
            >
              {heroTitle}{' '}
              <span className="text-cyan-light">{heroTitleHighlight}</span>
            </h1>
            <p className="text-white/80 mt-6 text-lg max-w-2xl mx-auto"
              {...(settings?._id ? ve(settings._id, 'siteSettings', 'healthPageDescription') : {})}
            >
                ...
                <Card className="group h-full border-0 shadow-lg shadow-marine/5 hover:shadow-xl hover:shadow-cyan/10 transition-all duration-500 overflow-hidden bg-white hover:-translate-y-1">
                  <CardContent className="p-6 space-y-4">
                    ...
                    <h3 className="text-lg font-bold text-marine group-hover:text-royal transition-colors"
                      {...(article._id ? ve(article._id, 'healthArticle', 'title') : {})}
                    >
                      {article.title}
                    </h3>
                    <p className="text-marine/60 text-sm leading-relaxed"
                      {...(article._id ? ve(article._id, 'healthArticle', 'description') : {})}
                    >
                      {article.description}
                    </p>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal>
            <div className="text-center mb-12"
              {...(settings?._id ? ve(settings._id, 'siteSettings', 'healthFaqTitle') : {})}
            >
              <h2 className="text-2xl sm:text-3xl font-bold text-marine">{faqTitle}</h2>
              ...
              {displayFaqs.map((faq: any, i: number) => (
                ...
                >
                  <AccordionTrigger className="text-marine font-semibold text-left hover:no-underline py-5 [&>svg]:text-cyan"
                    {...(settings?._id ? ve(settings._id, 'siteSettings', `healthFaqs[${i}].q`) : {})}
                  >
                    {faq.q}
                  </AccordionTrigger>
                  <AccordionContent className="text-marine/70 leading-relaxed pb-5"
                    {...(settings?._id ? ve(settings._id, 'siteSettings', `healthFaqs[${i}].a`) : {})}
                  >
                    {faq.a}
                  </AccordionContent>
                </AccordionItem>
